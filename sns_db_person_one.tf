...
resource "aws_sns_topic" "sns_db_person_one" {
  name         = "sns_db_person_one"
  display_name = "SNS DB - Person One"
  ...
  provisioner "local-exec" {
    command = "aws sns subscribe --topic-arn ${self.arn} --protocol email --notification-endpoint ${var.sns_email_db_person_one}"
    when    = create
  }
