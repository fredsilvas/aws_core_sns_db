...
resource "aws_sns_topic" "sns_db_person_two" {
  name         = "sns_db_person_two"
  display_name = "SNS DB - Person Two"
  ...
  provisioner "local-exec" {
    command = "aws sns subscribe --topic-arn ${self.arn} --protocol email --notification-endpoint ${var.sns_email_db_person_two}"
    when    = create
  }
